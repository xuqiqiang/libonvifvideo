...
int compare_crc(char *filename, unsigned int *crcReq)  
{  
    FILE *sp = NULL;   
    //unsigned int calcuCrc = 0xffffffff;  
    unsigned char buf[1024];  
    unsigned int  DataLen;  
      
    if(filename == NULL)  
    {  
        DPRINT(VIDEO_LOG_ERR,"filename is null\n");  
        return -1;  
    }  
    ...
    if(sp == NULL)  
    {  
        DPRINT(VIDEO_LOG_ERR,"open file fail\n");  
        return -1;  
    }   
        ...
        {  
                ...
                {  
                        ...
                        DataLen = fread(buf, 1, sizeof(buf), sp);  
						StringCrc32(buf, crcReq,DataLen);
                }  
        }  
    DPRINT(VIDEO_LOG_DEBUG,"In %s: calcuCrc is 0x%x\n", __FUNCTION__, *crcReq);  
